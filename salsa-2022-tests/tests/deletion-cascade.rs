...
fn basic() {
    ...
    db.assert_logs(expect![[r#"
        [
            "intermediate_result(MyInput(Id { value: 1 }))",
            "salsa_event(WillDiscardStaleOutput { execute_key: create_tracked_structs(MyInput { [salsa id]: 0 }), output_key: MyTracked(2) })",
            "salsa_event(DidDiscard { key: MyTracked(2) })",
            "salsa_event(DidDiscard { key: field(MyTracked { [salsa id]: 2 }) })",
            "salsa_event(DidDiscard { key: contribution_from_struct(MyTracked { [salsa id]: 2 }) })",
            "salsa_event(DidDiscard { key: MyTracked(5) })",
            "salsa_event(DidDiscard { key: field(MyTracked { [salsa id]: 5 }) })",
            "salsa_event(DidDiscard { key: copy_field(MyTracked { [salsa id]: 5 }) })",
            "salsa_event(WillDiscardStaleOutput { execute_key: create_tracked_structs(MyInput { [salsa id]: 0 }), output_key: field(MyTracked { [salsa id]: 2 }) })",
            "final_result(MyInput(Id { value: 1 }))",
        ]"#]]);
}
