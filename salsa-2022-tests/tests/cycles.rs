...
fn cycle_memoized() {
    ...
    let expected = expect![[r#"
        [
            "memoized_a(MyInput { [salsa id]: 0 })",
            "memoized_b(MyInput { [salsa id]: 0 })",
        ]
    "#]];
...
fn cycle_volatile() {
    ...
    let expected = expect![[r#"
        [
            "volatile_a(MyInput { [salsa id]: 0 })",
            "volatile_b(MyInput { [salsa id]: 0 })",
        ]
    "#]];
    expected.assert_debug_eq(&cycle.all_participants(&db));
}
...
fn inner_cycle() {
    ...
    let expected = expect![[r#"
        [
            "cycle_a(ABC { [salsa id]: 0 })",
            "cycle_b(ABC { [salsa id]: 0 })",
        ]
    "#]];
